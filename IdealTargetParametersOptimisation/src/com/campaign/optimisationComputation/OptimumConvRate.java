				...
				for (String campId : campIds) {
					for (ParameterConvRateDTO dto1 : dtoList) {
						if (dto1.getCampaignId().equals(campId)) {
							...
							convrate = Double
									.parseDouble(dto1.getConversions())
					...
					if (integercheck == (int) integercheck) {
						...
						IIIrdQuartile = convrateList
								.get(3 * (int) integercheck)
								+ convrateList.get(3 * (int) integercheck + 1)
								/ 2;
					}
					...
					if (m == 1 && n == 0)
						InsertOptimisedParametersData.OSConvRateoptimisation(
								campId, "DBM", parameterP1Map + parameterP2Map);
					...
					if (m == 2 && n == 1)
						InsertOptimisedParametersData
								.SegmentConvRateoptimisation(campId, "Adwords",
										parameterP1Map + parameterP2Map);
