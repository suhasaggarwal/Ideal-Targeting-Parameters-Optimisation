				...
				for (String campId : campIds) {
					for (ParameterConvCostDTO dto1 : dtoList) {
						if (dto1.getCampaignId().equals(campId)) {
							...
							convcost = Double.parseDouble(dto1.getSpent())/ Double.parseDouble(dto1.getConversions());
					...
					if (integercheck == (int) integercheck) {
						...
						IIIrdQuartile = convcostList.get(3 * (int) integercheck)
								+ convcostList.get(3 * (int) integercheck + 1) / 2;
					}
					...
					if (m == 1 && n == 0)
						InsertOptimisedParametersData.OSConvCostoptimisation(campId,
					...
					if (m == 2 && n == 0)
						InsertOptimisedParametersData.SegmentConvCostoptimisation(
								campId, "DBM", parameterP1Map + parameterP2Map);
